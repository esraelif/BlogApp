    ...
    return (
        <div className='sticky top-[6rem]'>
            <div className='border-b border-gray-400 pb-7'>
                <h2 className='font-semibold'>Discover The Others</h2>
                ...
                <button className='text-green-600 text-sm py-3 hover:text-black1'>Discover more topics</button>

            </div>
            <div className='flex items-center flex-wrap gap-3 leading-3 pt-8'>
                {discoverActions.map((item, i) => (
                    <button key={i} className='text-md text-black1 hover:text-green-700'>{item}</button>
                ))}
            </div>
        </div>
    );
